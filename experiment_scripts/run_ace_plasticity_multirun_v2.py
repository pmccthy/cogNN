"""
v2 of the ACE / ABEF plasticity-switch multirun script.

Changes vs v1 (run_context_signal_abef_plasticity_switch_multirun.py):

Recording
---------
* within_trial_actions  — binary lick (0) / no-lick (1) at every within-trial
  timestep.  Complements within_trial_lick_probs / within_trial_values.
* trial_stim_lick_counts — count of lick actions during the stimulus window
  (mirrors trial_reward_lick_counts for the reward window).
* within_trial_states records from the current observation (obs) rather than
  info['state_name'], avoiding a 1-step shift.

Plot labels
-----------
* trial_lick_probs    → "Lick prob at last stim-window timestep"
* trial_reward_lick_counts → "Lick count in reward window"

Time-resolved decoding (new)
-----------------------------
* decode_timeresolved() — at each timestep in an aligned trial window
  ([n_pre_iti ITI ts] + [stim window] + [reward window]), fits a logistic
  regression decoder (high vs low value) for within-pre, within-post,
  pre→post, post→pre.
* plot_timeresolved_decoder() / plot_agg_timeresolved_decoder()
* plot_tdr_design_matrix() — visualise TDR regression design matrix

All shared utilities are in cog_nn.analysis_utils.

Usage:
    python run_ace_plasticity_multirun_v2.py \\
        --n_runs 10 --n_train_reversals 1 --n_test_reversals 1 \\
        --trials_per_phase 500 \\
        --task_pkl task_data/reversal_ace_singlerev_plast_tpp500_seed42.pkl \\
        --plot_stims 0,2,4 --no_context \\
        --results_dir results/21_04_26_singlerev_ace_plasticity_nocontext_v2
"""
...
from cog_nn.analysis_utils import (
    ...
    plot_timeresolved_decoder, plot_tdr_design_matrix,
    aggregate_plots,
)


# ── Training (v2: adds within_trial_actions + trial_stim_lick_counts) ─────────

def train(env, model, cfg, trial_structure, phase_boundaries, seed):
    """
    Like v1 train() but additionally records:
      * within_trial_actions  — binary lick/no-lick per timestep
      * trial_stim_lick_counts — lick count in the stimulus window
    Also derives within_trial_states from current obs (not info['state_name']).
    """
    ...
    metrics = {
        ...
        'trial_stim_lick_counts':   {k: [] for k in stim_keys},
        'trial_timesteps':  [],
        'trial_indices':    [],
    ...
    def record_step(t_idx, obs, action, action_prob, value, rnn_out, reward, info):
        ...
        # Derive state name from current obs (avoids 1-step shift from info)
        state_name = (OBS_STATE_NAMES[state_idx]
                      if state_idx < len(OBS_STATE_NAMES) else '?')
        ...
        if trial_info is not None:
            ...
            if t_idx == td['trial_start']:
                ...
                for key in ('within_trial_lick_probs', 'within_trial_values',
                            ...
                            'within_trial_actions', 'hidden_states'):
                    metrics[key][trial_info] = []
                _stim_lick_buf[trial_info] = 0
            ...
            if trial_info in metrics['within_trial_lick_probs']:
                ...
                metrics['within_trial_actions'][trial_info].append(int(action))
            ...
            if trial_info in _stim_lick_buf and t_idx in td['stim_window']:
                if action == 0:
                    _stim_lick_buf[trial_info] += 1
            if t_idx == td['stim_window'][-1]:
                k = idx_to_stim.get(td['stimulus'])
                if k and trial_info in _stim_lick_buf:
                    metrics['trial_stim_lick_counts'][k].append(
                        _stim_lick_buf.pop(trial_info))
    ...
    metrics_numpy = {
        ...
        'trial_stim_lick_counts':   {k: np.array(v) for k, v in metrics['trial_stim_lick_counts'].items()},
        ...
        'within_trial_actions':     metrics['within_trial_actions'],
...
# ── Analysis (adds time-resolved decoder on top of base) ──────────────────────

def analyse(metrics_numpy, model, cfg, trial_structure):
    results = _analyse_base(metrics_numpy, model, cfg, trial_structure)

    _log("  Analysis: time-resolved decoder...", indent=1)
    hidden_size  = cfg['hidden_size']
    ...
    tr_dec = decode_timeresolved(
        metrics_numpy, trial_structure,
        readout_indices, non_readout_indices,
        W_actor, b_actor, W_critic, b_critic,
        results['PRE_HIGH'], results['PRE_LOW'],
        results['POST_HIGH'], results['POST_LOW'],
        n_pre_iti=3,
    )
    results['timeresolved_decoder'] = tr_dec
    return results
...
def main():
    ...
    parser.add_argument('--lick_no_reward',      type=float, default=-1.0,
                        help='Reward given for licking when no reward is available '
                             '(default -1.0; set to 0.0 for omission-only, no punishment)')
    parser.add_argument('--overwrite',           action='store_true')
    parser.add_argument('--run_indices',         type=str,   default='')
    parser.add_argument('--results_dir',  type=str,
                        default='results/run_ace_plasticity_v2')
    ...
    cfg = {
        ...
        'lick_no_reward':     args.lick_no_reward,
    }

    _ro_str    = f'partialro{readout_size}'
    _h_str     = f'_h{hidden_size}'
    ...
    _punish_str = (f'_punish{args.lick_no_reward:g}'.replace('-', 'n')
                   if args.lick_no_reward != -1.0 else '')
    variant_name = (f'abef_plswitch{_ctx_str}'
                    f'_train{args.n_train_reversals}rev'
                    f'_test{args.n_test_reversals}rev'
                    f'_tpp{args.trials_per_phase}{_win_str}{_punish_str}{_h_str}_{_ro_str}_v2')
    ...
    for run_idx in run_indices:
        ...
        env = ReversalABCDEFMultiTimestepEnv(
            ...
            reward_lick=1.0, lick_no_reward=cfg['lick_no_reward'], no_lick=0.0)
        state_size  = env.observation_space.shape[0]
        action_size = env.action_space.n
        ...
        plot_tdr_design_matrix(results, run_dir, variant_label)
        _log("    tdr_design_matrix.png", indent=2)
        ...
        plot_timeresolved_decoder(results, run_dir, variant_label, plot_stims, run_idx, seed)
        _log("    timeresolved_decoder.png", indent=2)
        ...
        lick_value_data = {
            ...
            'trial_reward_lick_counts':{k: metrics_numpy['trial_reward_lick_counts'][k]
                                        for k in 'ABCDEF'},
            'trial_stim_lick_counts':  {k: metrics_numpy['trial_stim_lick_counts'][k]
                                        for k in 'ABCDEF'},
        ...
        run_results = {
            ...
            'timeresolved_decoder': results.get('timeresolved_decoder'),
        ...
        _log(f"  Run {run_idx+1} done in {time.time()-_t_run:.0f}s", indent=1)
