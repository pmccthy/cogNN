...
# TODO: implement A2C with:
#        - Poisson distribution for action selection (for continuous action spaces)

class A2CContinuousAction:
    """
    A2C agent with continuous action selection (actor outputs event rate parameter for Poisson distribution).
    """
    def __init__(self, actor, critic, actor_optimizer, critic_optimizer, 
                 gamma=0.9, device='cpu'):
        """
    ...
    def select_action(self, state):
        """
        Select an action using actor network to predict rate parameter for Poisson distribution, then sample.
        """
        if isinstance(state, np.ndarray):
            state = torch.from_numpy(state).float()
        ...
        # Get action rate parameter from actor
        action_rate = self.actor(state)
        
        # Sample action from Poisson distribution
        action = np.random.poisson(action_rate.item())
        
        return action, action_rate.item()
...
class A2CAgentAsymmetricLR:
    """
    Actor-Critic agent implementing the Advantage Actor-Critic (A2C) algorithm
    with asymmetric learning rates for positive and negative reward prediction errors.
    ...
    Updates are performed using advantage estimates: A = R + γV(s') - V(s)
    with different learning rate multipliers for positive vs negative advantages.
    """
    
    def __init__(self, actor, critic, actor_optimizer, critic_optimizer, 
                 gamma=0.9, device='cpu', positive_lr_multiplier=1.0, 
                 negative_lr_multiplier=1.0):
        """
        Initialize A2C agent with asymmetric learning rates.
        ...
        Args:
            ...
            gamma: Discount factor for future rewards
            device: Device to run on ('cpu' or 'cuda')
            positive_lr_multiplier: Multiplier for positive reward prediction errors
            negative_lr_multiplier: Multiplier for negative reward prediction errors
        """
        self.actor = actor
        self.critic = critic
        self.actor_optimizer = actor_optimizer
        ...
        self.gamma = gamma
        self.device = device
        self.positive_lr_multiplier = positive_lr_multiplier
        self.negative_lr_multiplier = negative_lr_multiplier
    ...
    def select_action(self, state, deterministic=False, policy_clip=None):
        ...
        Args:
            ...
            policy_clip: Optional clipping value for action probabilities
        ...
        # Optionally clip probabilities
        if policy_clip is not None:
            action_probs = torch.clamp(action_probs, policy_clip, 1 - policy_clip)
    ...
    def update(self, states, actions, rewards, next_states, dones):
        """
        Update Actor and Critic networks using A2C algorithm with asymmetric learning rates.
        ...
        # Apply asymmetric learning rate multipliers to advantages
        scaled_advantages = (
            advantages * positive_mask * self.positive_lr_multiplier +
            advantages * negative_mask * self.negative_lr_multiplier
        )
        
        # Actor loss: -log_prob * scaled_advantage (policy gradient)
        actor_loss = -(log_probs * scaled_advantages.detach()).mean()
        
        # Critic loss: apply asymmetric multipliers to squared errors
        # Scale the loss contribution based on advantage sign
        critic_errors = target_values - values
        critic_loss_positive = (
            (critic_errors.pow(2) * positive_mask * self.positive_lr_multiplier).mean()
        )
        critic_loss_negative = (
            (critic_errors.pow(2) * negative_mask * self.negative_lr_multiplier).mean()
        )
        critic_loss = critic_loss_positive + critic_loss_negative
...
class TDLearnerWithBernoulliActionSelection:
    """
    Tabular TD(0) Value Learner with Bernoulli action selection for discrete state spaces.
    """
    def __init__(self, state_size, lr=0.1, gamma=0.9):
        """
        Initialize Tabular Value Learner with action selection.
    ...
    def select_action(self, state, action_size, beta=1.0, bias=0):
        """
        Select an action using Bernoulli distribution based on state value.
        
        Args:
            state: Current discrete state (integer)
            action_size: Number of discrete actions (assumed to be 2 for Bernoulli)
            beta: Inverse temperature parameter for softmax
            bias: Bias term added to value before softmax
        ...
        """
        value = self.value_table[state]
        logit = beta * (value + bias) # logit for lick action
        # softmax to get probabilities
        action_prob = 1 / (1 + np.exp(-logit))
        action = np.random.choice(action_size, p=[action_prob, 1 - action_prob])
...
class TDLearnerWithPoissonActionSelection:
    """
    Tabular TD(0) Value Learner with Poisson action selection for discrete state spaces.
    """
    def __init__(self, state_size, lr=0.1, gamma=0.9):
        """
        Initialize Tabular Value Learner with action selection.
    ...
    def select_action(self, state, beta=1.0, bias=0):
        """
        Select an action using Poisson distribution based on state value.
        
        Args:
            state: Current discrete state (integer)
            action_size: Number of discrete actions (assumed to be 2 for Poisson)
            lambda_: Rate parameter for Poisson distribution
        ...
        """
        value = self.value_table[state]
        # predict rate parameter from value
        lambda_ = np.exp(beta * (value + bias))
        # sample action from Poisson distribution
        action = np.random.poisson(lambda_)
        # compute probability of selected action
        action_prob = (lambda_ ** action) * np.exp(-lambda_) / np.math.factorial(action)
